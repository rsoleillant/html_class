...
class menu{
  ...
  public function getId()
 {
  ...
  public function htmlValue()
  {
    ...
    if($this->bool_multiple_actif)
    {
     $stri_javascript=$this->constructJavascript();
     //setcookie("pageId");
    }
 ...
 public function constructJavascript()
 {
   $obj_javascripter=new javascripter();
    $int_page_id=$this->getPageId();
  if(($this->bool_multiple_actif)&&(menu::$int_menu==1))
  {
        
    $obj_javascripter->addFunction("
   
   /******************************
    *Permet de faire en sorte que chaque formulaire 
    *transmette en plus en post un champ pageId contenant
    *l'identifidant de la page
    ******************************/                
   function transmitPageId()
   {
    //récupération de tous les formulaire
    var arra_form=document.getElementsByTagName('form');
    var i;
    var nb_element=arra_form.length;
    //création de l'input contenant le pageId
    var hidden=document.createElement('input');
        hidden.setAttribute('type','hidden');
        hidden.setAttribute('name','pageId');
        hidden.setAttribute('value','$int_page_id');

     for(i=0;i<nb_element;i++)
     {
     //clonage du hidden pour avoir des objets différents
      var clone=hidden.cloneNode(true);
     //ajout du clone au formulaire 
      //arra_form[i].insertBefore(clone,arra_form[i].lastChild);
      var form=arra_form[i];
      var nb_input=form.elements.length;
      var j=0,trouve=false;
      //recherche si l'input est déjà présent
      while((j<nb_input)&&(!trouve))
      {
       if(form.elements[j].getAttribute('name')=='pageId')
       {trouve=true;}
       j++;
      }
      //si l'input n'a jamais été rattaché au formulaire, on l'attache
      if(!trouve)
      {arra_form[i].appendChild(clone);}
    ...
    //initialisation de la transmission de l'identifiant de page
    var body =document.getElementsByTagName('body')[0];
    //on change le style du curseur pour faire apparaite le curseur de chargement
    body.style.cursor = 'wait';
   
    //on lance la transmission du pageId lorsque la page est finie de chargée
    
      if(window.addEventListener)
        {window.addEventListener('load', transmitPageId, false);}//pour firefox
      else
        {window.attachEvent('onload', transmitPageId);}//pour ie ... 
    
    //on lance la transmission de l'id également le plus tot possible si l'utilisateur n'attend pas la fin du chargement
    transmitPageId();
    ");
